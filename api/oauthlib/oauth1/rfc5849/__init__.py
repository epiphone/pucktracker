    ...
    * @dummy_access_token
    ...
    def verify_request(self, uri, http_method='GET', body=None,
        ...
        if not self.validate_timestamp_and_nonce(request.client_key,
                ...
                return False, request, "validate_timestamp_and_nonce mokasi."
        ...
        if not v:
            ...
            reasons = """
            [Failure] OAuthLib request verification failed.
            Valid client:\t%s
            Valid token:\t%s\t(Required: %s
            Valid realm:\t%s\t(Required: %s)
            Valid callback:\t%s
            Valid verifier:\t%s\t(Required: %s)
            Valid signature:\t%s)
            """ % (valid_client, valid_resource_owner, require_resource_owner,
                valid_realm, require_realm, valid_redirect, valid_verifier,
                require_verifier, valid_signature)
        return v, request, reasons
